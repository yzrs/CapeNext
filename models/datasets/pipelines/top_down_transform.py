...
class TopDownBboxCrop:
    ...
    def __call__(self, results):
        ...
        # 计算原图中可截取的有效区域
        x_start = max(new_x, 0)
        x_end = min(new_x + s, img_w)
        y_start = max(new_y, 0)
        y_end = min(new_y + s, img_h)

        # 计算目标图像中的填充位置
        dst_x_start = x_start - new_x
        dst_x_end = dst_x_start + (x_end - x_start)
        dst_y_start = y_start - new_y
        dst_y_end = dst_y_start + (y_end - y_start)

        # 创建目标图像（自动处理单通道和彩色图像）
        if img.ndim == 3:
            cropped_img = np.zeros((s, s, img.shape[2]), dtype=img.dtype)
        else:
            cropped_img = np.zeros((s, s), dtype=img.dtype)

        # 将原图有效区域复制到目标图像
        cropped_img[dst_y_start:dst_y_end, dst_x_start:dst_x_end] = img[y_start:y_end, x_start:x_end]
        results['img'] = cropped_img
        return results


@PIPELINES.register_module()
class TopDownMaskImage:
    def __init__(self):
        self.ratio = 1
        self.use_padding = True
    ...
    def __call__(self, results):
        ...
        if img.ndim == 3:
            cropped_img = np.zeros((img_h, img_w, img.shape[2]), dtype=img.dtype)
        else:
            cropped_img = np.zeros((img_h, img_w), dtype=img.dtype)

        # 将原图有效区域复制到目标图像
        cropped_img[dst_y_start:dst_y_end, dst_x_start:dst_x_end] = img[y:y+h, x:x+w]
        results['img'] = cropped_img
