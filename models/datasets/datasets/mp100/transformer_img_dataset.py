...
class TransformerImgPoseDataset(TransformerBaseDataset):
    ...
    def update_points_names(self, category):
        updated_point_names = rename_points_descriptions(category, test_type="regular_test")

        if len(updated_point_names) > 0:
            self.cats[category['id']]['keypoints'] = updated_point_names

    def get_points_names_tokens(self):
        print("Adding point names with regular_test.")
        ...
        for category_id in self.cats.keys():
            ...
            # point_names = point_names.tolist()
            # category_name = self.cats[category_id]['name'].replace('_', ' ')
            # for i, point_name in enumerate(point_names):
            #     point_names[i] = point_name + ' of a ' + category_name
            # point_names = np.array(point_names)

            self.cats_points_descriptions[category_id] = point_names
        # print("cats_points_descriptions:", self.cats_points_descriptions)
        ...
        for obj in objs:
            ...
            if os.path.exists(image_file):
                ...
                rec.append({
                    ...
                    'category_name': self._ind_to_class[category_id].replace('_', ' '),
    ...
    # getItem function for additional suppport image
    def __getitem__(self, idx):
        ...
        for sample_obj in sample_obj_list:
            ...
            # pipeline of CapeX: delete the support info
            ...
            # Here we remove this process to validate the influence of support image
            # How about CapeNext + image ?
