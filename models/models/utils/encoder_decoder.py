...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        # self.use_self_attention_contrast = True
        # if self.use_self_attention_contrast:
        #     self.self_attention_contrast = nn.MultiheadAttention(embed_dim=self.d_model, num_heads=self.nhead, dropout=0.1)
        #     self.contrast_norm = nn.LayerNorm(self.d_model)
        #     print("use self_attention_contrast to improve image feature")
    ...
    def forward(self, src, mask, support_embed, pos_embed, support_order_embed,
        ...
        # 此处可以考虑添加Joint-Within-Attn
        # if self.use_self_attention_contrast:
        #     contrast_support_embed, _ = self.self_attention_contrast(query=refined_support_embed,
        #                                                              key=refined_support_embed,
        #                                                              value=refined_support_embed)
        #     refined_support_embed = refined_support_embed + contrast_support_embed
        #     refined_support_embed = self.contrast_norm(refined_support_embed)
